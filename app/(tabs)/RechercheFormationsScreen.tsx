...
const VideoFormationItem = ({ item, index, onPress, participantCounts, userDemandes, isAdmin, isInView }) => {
  ...
  // Video processing functions (from FormationScreen)
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1]) {
        ...
        return {
          id: videoId,
          type: 'youtube',
          embedUrl: `https://www.youtube.com/embed/${videoId}?modestbranding=1&rel=0&controls=1&showinfo=0&fs=1&autoplay=1&mute=1`,
  ...
  // Auto-play logic based on visibility
  useEffect(() => {
    if (videoData.id && videoRef.current) {
      if (isInView && !videoPaused) {
        videoRef.current.playAsync();
      } else {
        videoRef.current.pauseAsync();
      }
    }
  }, [isInView, videoPaused, videoData.id]);

  // Handle video playback when item comes into view
  useEffect(() => {
    setVideoPaused(!isInView);
  }, [isInView]);
      ...
      return (
        <View style={styles.videoPlayerContainer}>
          <Video
            ...
            useNativeControls
            resizeMode="contain"
            shouldPlay={isInView && !videoPaused}
...
const RechercheFormationsScreen = (props, { route }) => {
  ...
  // (Including notifications setup, data fetching, filtering, etc.)
  ...
  // (Including fetchFormations, applyFilters, handleLogout, etc.)
  ...
  // Filter Tabs Component (keeping existing implementation)
  const FilterTabs = ({ 
    filters, 
    activeFilters, 
  ...
  // Enhanced renderFormationItem with video support
  const renderFormationItem = ({ item, index }) => {
    const isInView = viewableItems.some(viewableItem => viewableItem.item.id === item.id);
...
const styles = StyleSheet.create({
  // Existing styles remain the same, plus new video-related styles...
